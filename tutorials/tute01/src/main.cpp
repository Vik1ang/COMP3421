...
namespace {
    void on_resize(GLFWwindow *_w, int width, int height) {
    ...
    mesh_t make_mesh() {
        ...
        glBufferData(GL_ARRAY_BUFFER, static_cast<GLintptr>(verts.size() * sizeof(vertex_t)), &verts[0], GL_STATIC_DRAW);
...
int main() {
    ...
    // TODO: Step 2
    GLFWwindow *window = make_window(SCR_WIDTH, SCR_HEIGHT, WIN_TITLE);
    if (!window) {
    ...
    // TODO: Step 4
    const std::vector<GLuint> shaders = std::vector<GLuint>{
            make_shader(VERT_PATH, GL_VERTEX_SHADER),
            make_shader(FRAG_PATH, GL_FRAGMENT_SHADER)
    };

    // TODO: Step 5
    const GLuint render_program = make_program(shaders);
