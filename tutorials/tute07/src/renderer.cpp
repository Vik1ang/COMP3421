...
renderer_t make_renderer(const glm::mat4 &projection) {
    ...
    GLint sun_loc = chicken3421::get_uniform_location(render_program, "sun");

    return renderer_t{ projection, render_program, p_loc, v_loc, m_loc, sun_loc };
}
...
void render(const renderer_t &renderer, const scene_t &scene) {
    ...
    glUniform3fv(renderer.sun_loc, 1, glm::value_ptr(scene.sunlight));
