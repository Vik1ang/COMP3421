...
OpenGL允许我们**同时绑定多个缓冲**，只要它们是**不同的缓冲类型**。我们可以使用`glBindBuffer`函数把新创建的缓冲绑定到`GL_ARRAY_BUFFER`目标上
...
`glCreateProgram`函数创建一个程序，并**返回新创建程序对象的ID引用**
...
> 当目标是`GL_ELEMENT_ARRAY_BUFFER`的时候，`VAO`会储存`glBindBuffer`的函数调用。
> 
