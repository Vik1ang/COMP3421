...
glm::mat4 animate(const bspline_t &bspline, float now) {
    ...
    return glm::translate(glm::translate(glm::mat4(1), glm::vec3(2.0f)), tr);
//    return glm::translate(glm::translate(glm::mat4(1), glm::vec3(2.0f)), tr);
}
...
glm::mat4 animate(const lemniscate_t &lemni, float now) {
    ...
    std::cout << "cos(t): " << std::cos(t) << std::endl;

    return glm::translate(glm::mat4(1), glm::vec3{std::sin(t), 0, std::cos(t)});
}

glm::mat4 animate(const tract_t &track, float now) {
    ...
    while (!animatables.empty()) {
        ...
        switch (n->kind) {
            ...
            case node_t::SARU:
                ...
                break;
            case node_t::TRACK:
                n->transform = animate(animator.track_animation, now);
                break;
            default:
                break;
        }
